...
from churnq_agents.webhooks import stripe as stripe_webhooks
from churnq_agents.routers import agents as agents_router
from churnq_agents.jobs import queue
...
app = FastAPI(title="ChurnQ Agents", version="0.1.0", lifespan=lifespan)
...
def run():
    ...
    uvicorn.run("churnq_agents.main:app", host="0.0.0.0", port=port, reload=True)
